...
// “<XY>” in “1”, “2” and “u” lines, where unchanged is “.” not space. :contentReference[oaicite:7]{index=7}
type XYFlag struct {
	Staged   State // X ('.' if unchanged in index)
	Unstaged State // Y ('.' if unchanged in worktree)
}
...
type ChangedEntry struct {
	...
	ModeH string          // <mH> filemode in HEAD (octal string)
	ModeI string          // <mI> filemode in index (octal string)
	ModeW string          // <mW> filemode in worktree (octal string)
...
type UnmergedEntry struct {
	...
	Sub   SubmoduleStatus // 4-character submodule field
	Mode1 string          // <m1> mode of stage 1 (common base)
	Mode2 string          // <m2> mode of stage 2 (ours)
	Mode3 string          // <m3> mode of stage 3 (theirs)
	ModeW string          // <mW> mode in worktree
